...
module.exports = (app) => {
    ...
    app.post("/users", async (req, res) => {
        ...
        if ((req.body.role === "admin" || req.body.role === "instructor") && !isAdmin) {
            res.status(403).send({ error: "Invalid authentication to create privileged user"})
            return;
        }
    ...
    app.post("/users/login", async (req, res) => {
        ...
        try {
            ...
            let user = await Users.findOne({ email: req.body.email }).select("password").exec();
